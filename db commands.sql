...
select to_char(designation)  from employee where emp_id=20101001;
select get_designation(20101001) from dual ;
...
insert into donor(name,email,gender,address,phone,occupation) values('tdfyg','tdfyg','Male','tdfyg','tdfyg','tdfyg');
select * from donor;
--select max(substr(Donor_id,4)) from donor;
commit;

delete from donor where donor_id='Don201120111028';
---- generate donor_id
set serveroutput on;
...
--insert into volunteer(name,email,gender,address,phone,occupation) values('tdfyg','tdfyg','Male','tdfyg','tdfyg','tdfyg');
select * from volunteer;
commit;
--select substr(to_char(sysdate,'YYYYMMDD'),3,4)  from dual;
...
END;
/
select max(substr(volunteer_id,-4))  from volunteer ;
--- Calling this from a trigger
...
insert into food(name,purchase_date,expire_date,qty,supplier,unit_price) values('a',TO_DATE('01/10/2000', 'dd/mm/yyyy'),TO_DATE('12/10/2010', 'dd/mm/yyyy'),2,'1q',3);
------GENERATE FOOD ID
set serveroutput on;
...
/
select * from employee;
select lpad('e1006',1) from dual;
select amount,doctor_id from doctor,designation where speciality=designation_name;

select doctor.*,show_Doctor_Salary(doctor_id) from doctor;
---------show Doctor salary
...
exec add_employee('try.txt');
exec add_employee('csv','try.txt');

exec add_employee('E:\csv','try.txt');
...
delete from employee where email='a@k.ppl';
commit;
select * from doctor;
...
select * from HEALTH_PRODUCT;
select * from HEALTH_PRODUCT where name='oo' and purchase_date > add_months(sysdate,-6) order by purchase_date;